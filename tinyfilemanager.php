...
if (!isset($use_auth)) $use_auth = true;

// Login user name and password
// Users: array('Username' => 'Password', 'Username2' => 'Password2', ...)
// Generate secure password hash - https://tinyfilemanager.github.io/docs/pwd.html
if (!isset($auth_users)) $auth_users = array(
...
if (!isset($readonly_users)) $readonly_users = array(
    'user'
);

// Global readonly, including when auth is not being used
if (!isset($global_readonly)) $global_readonly = false;

// user specific directories
// array('Username' => 'Directory path', 'Username2' => 'Directory path', ...)
if (!isset($directories_users)) $directories_users = array();

// Enable highlight.js (https://highlightjs.org/) on view's page
if (!isset($use_highlightjs)) $use_highlightjs = true;

// highlight.js style
// for dark theme use 'ir-black'
if (!isset($highlightjs_style)) $highlightjs_style = 'vs';

// Enable ace.js (https://ace.c9.io/) on view's page
if (!isset($edit_files)) $edit_files = true;

// Default timezone for date() and time()
// Doc - http://php.net/manual/en/timezones.php
if (!isset($default_timezone)) $default_timezone = 'Etc/UTC'; // UTC
...
if (!isset($path_display_mode)) $path_display_mode = 'full';

// Allowed file extensions for create and rename files
// e.g. 'txt,html,css,js'
if (!isset($allowed_file_extensions)) $allowed_file_extensions = '';

// Allowed file extensions for upload files
// e.g. 'gif,png,jpg,html,txt'
if (!isset($allowed_upload_extensions)) $allowed_upload_extensions = '';

// Favicon path. This can be either a full url to an .PNG image, or a path based on the document root.
// full path, e.g http://example.com/favicon.png
// local path, e.g images/icons/favicon.png
if (!isset($favicon_path)) $favicon_path = '';

// Files and folders to excluded from listing
// e.g. array('myfile.html', 'personal-folder', '*.php', '/path/to/folder', ...)
if (!isset($exclude_items)) $exclude_items = array();
...
if (!isset($online_viewer)) $online_viewer = 'google';

// Sticky Nav bar
// true => enable sticky header
// false => disable sticky header
if (!isset($sticky_navbar)) $sticky_navbar = true;

// Maximum file upload size
// Increase the following values in php.ini to work properly
// memory_limit, upload_max_filesize, post_max_size
if (!isset($max_upload_size_bytes)) $max_upload_size_bytes = 5000000000; // size 5,000,000,000 bytes (~5GB)

// chunk size used for upload
// eg. decrease to 1MB if nginx reports problem 413 entity too large
if (!isset($upload_chunk_size_bytes)) $upload_chunk_size_bytes = 2000000; // chunk size 2,000,000 bytes (~2MB)

// Possible rules are 'OFF', 'AND' or 'OR'
// OFF => Don't check connection IP, defaults to OFF
// AND => Connection must be on the whitelist, and not on the blacklist
// OR => Connection must be on the whitelist, or not on the blacklist
if (!isset($ip_ruleset)) $ip_ruleset = 'OFF';
...
if (isset($_SESSION[FM_SESSION_ID]['logged']) && !empty($directories_users[$_SESSION[FM_SESSION_ID]['logged']])) {
    $user_dir = $directories_users[$_SESSION[FM_SESSION_ID]['logged']];
    if (!is_array($user_dir)) {
        $wd = fm_clean_path(dirname($_SERVER['PHP_SELF']));
        $root_url =  $root_url . $wd . DIRECTORY_SEPARATOR . $user_dir;
    }
}
// clean $root_url
...
if (is_array($root_path)) {
    $root_path = array_map(function($p) {
        $p = rtrim($p, '\\/');
...
if (isset($_GET['copy'], $_GET['finish']) && !FM_READONLY) {
    ...
    // abs path from
    $from = fm_get_path($copy);
    // abs path to
    $dest = fm_get_path(FM_PATH);
    $dest .= '/' . basename($from);
    // move?
...
// check path
if ($path === false || (is_string($path) && !is_dir($path))) {
    if (is_array(FM_ROOT_PATH) && FM_PATH == '') {
        // Virtual root view
        $path = null;
    } else {
        fm_redirect(FM_SELF_URL . '?p=');
...
$objects = ($path && is_readable($path)) ? scandir($path) : array();
$folders = array();
$files = array();
if (is_array(FM_ROOT_PATH) && FM_PATH == '') {
    foreach (FM_ROOT_PATH as $idx => $rpath) {
        $folders[] = $idx . '-' . basename($rpath);
    }
} else {
...
?>
    <div class="path">
        <p><b>Copying</b></p>
        <p class="break-word">
            <strong>Source path:</strong> <?php echo fm_enc(fm_convert_win(fm_get_path($copy))) ?><br>
            <strong>Destination folder:</strong> <?php echo fm_enc(fm_convert_win(fm_get_path(FM_PATH))) ?>
        </p>
        <p>
...
/**
 * Resolve path for multiple roots
...
function fm_get_path($fragment)
{
    if (is_array(FM_ROOT_PATH)) {
        ...
        if (isset(FM_ROOT_PATH[$idx])) {
            ...
            return FM_ROOT_PATH[$idx] . ($rest !== '' ? '/' . $rest : '');
        }
        return false;
    }
    return FM_ROOT_PATH . ($fragment !== '' ? '/' . $fragment : '');
}
